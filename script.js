...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // カードのホバーエフェクト強化
    // ===============================
    const cards = document.querySelectorAll('.phase-card, .prep-card, .choice-card, .ref-card');
...
function autoExpandFirstPendingDay() {
    ...
    if (daysDiff >= 0 && daysDiff < 7) {
        const currentDayCard = document.querySelector(`[data-day="${daysDiff + 1}"]`);
        if (currentDayCard) {
...
function initializeAnimationObserver() {
    const animatedElements = document.querySelectorAll('.phase-card, .prep-card, .ref-card');
...
function saveUserPreferences() {
    const preferences = {
        expandedDays: Array.from(document.querySelectorAll('.day-content.active')).map(content => 
            content.parentElement.getAttribute('data-day')
        ),
        selectedChoice: document.querySelector('.choice-card.selected')?.classList.contains('adventure') ? 'adventure' : 'scenic'
    };
...
function loadUserPreferences() {
    ...
    if (saved) {
        ...
        if (preferences.selectedChoice) {
            const choiceCard = document.querySelector(`.choice-card.${preferences.selectedChoice}`);
            if (choiceCard) {
